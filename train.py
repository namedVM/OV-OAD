...
def main() -> None:
    ...
    # 针对分布式训练中的未使用分支（如 zero-shot 开关导致的分支），开启 find_unused_parameters
    ...
    if accelerator.is_main_process:
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                trainable_params.append(f"{name} ({param.numel()})")
            else:
                frozen_params.append(name)

        logger.info(f"可训练参数总量: {len(trainable_params)} 个标识符")
        n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        logger.info(f"可训练参数总计: {n_params / 1e6:.2f}M")

        if len(trainable_params) < 50:  # 如果不多，列出来方便核对
            logger.info(f"可训练列表: {', '.join(trainable_params)}")
        else:
            # 仅显示模块前缀
            prefixes = sorted(list(set(n.split(".")[0] for n in trainable_params)))
            logger.info(f"可训练模块前缀: {prefixes}")
