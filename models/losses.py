...
## focal loss, take from maskformer/modeling/criterion.py
def sigmoid_focal_loss(inputs, targets, num_masks=32, alpha: float = 0.25, gamma: float = 2):
    """
...
def dice_loss(inputs, targets, num_masks=None, threshold=0.0, topk_mask=None):
    ...
    inputs = inputs.flatten(1) # [256, 401408] 
    targets = targets.flatten(1) # [256, 401408]
    # embed()
    # exit()
    numerator = 2 * (inputs * targets).sum(-1) # [256]
    denominator = inputs.sum(-1) + targets.sum(-1) # 256
    loss = 1 - (numerator + 1) / (denominator + 1) # 256
    return loss.sum() / num_masks # tensor(10.7107, device='cuda:0')
